...
class PlaylistViewTestCase(TestCase):
    ...
    def test_show_detail_view(self):
        ...
        self.assertContains(response, f"{show.title}")  # look into the response html -> show.title
        context = response.context
        obj = context['object']
        self.assert_(obj.id, show.id)
    ...
    def test_movie_detail_view(self):
        ...
        self.assertEqual(response.status_code, 200) # 200
        self.assertContains(response, f"{movie.title}") # look into the response html -> show.title
        context = response.context
        obj = context['object']
        self.assert_(obj.id, movie.id)
    ...
    def test_search_none_view(self):
        ...
        self.assertContains(response, 'Realizar una Busqueda')
    ...
    def test_search_results_view(self):
        ...
        r_qs = context['object_list']
        self.assertQuerysetEqual(ply_qs.order_by('-timestamp'), r_qs.order_by('-timestamp'))
        self.assertContains(response, f"Buscar {query}")
